@props(['students', 'stats', 'totalStats' => null, 'showBalance' => false, 'editable' => false])
        ...
        @if($editable)
            <div wire:key="student-row-{{ $student->id }}" class="flex items-center justify-between gap-3 px-3 py-2 rounded hover:bg-gray-50">
                <div class="flex items-center gap-2 min-w-0">
                    ...
                    >
                        <button type="button" @click="open = ! open" :aria-expanded="open" title="Change status" class="flex items-center rounded p-1 hover:bg-gray-200">
                            <x-student-status-dot :status="$student->status" />
                        </button>

                        <div x-show="open" x-cloak x-transition class="absolute left-0 top-7 z-30 w-60 rounded-lg border border-gray-200 bg-white p-3 text-left shadow-lg">
                            <label class="mb-1 block text-xs font-medium text-gray-500">Status</label>
                            <select x-model="status" class="w-full rounded-md border-gray-200 bg-white text-sm text-gray-700 focus:border-gray-400 focus:ring-gray-300">
                            ...
                            <div x-show="status === '{{ $holidayValue }}'" x-cloak class="mt-2 grid grid-cols-2 gap-2">
                                <div>
                                    <label class="mb-1 block text-xs font-medium text-gray-500">From</label>
                                    <input type="date" x-model="start" class="w-full rounded-md border-gray-200 bg-white text-sm text-gray-700 focus:border-gray-400 focus:ring-gray-300">
                                </div>
                                <div>
                                    <label class="mb-1 block text-xs font-medium text-gray-500">To</label>
                                    <input type="date" x-model="end" class="w-full rounded-md border-gray-200 bg-white text-sm text-gray-700 focus:border-gray-400 focus:ring-gray-300">
                                </div>
                            </div>

                            {{-- Any non-active status → an optional note --}}
                            <div x-show="status !== '{{ $activeValue }}'" x-cloak class="mt-2">
                                <label class="mb-1 block text-xs font-medium text-gray-500">Comment <span class="text-gray-400">(optional)</span></label>
                                <textarea x-model="note" rows="2" placeholder="e.g. Moved abroad, stopped in May" class="w-full rounded-md border-gray-200 bg-white p-2 text-sm text-gray-700 focus:border-gray-400 focus:ring-gray-300"></textarea>
                            </div>
